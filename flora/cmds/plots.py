...
def show_plots(root_path: Path):
    from flora.plotting.plot_tools import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'flora plots add -d [dir]' and remove with"
        + " 'flora plots remove -d [dir]'"
        + " Scan and check plots with 'flora plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'flora init'")
...
def create_cmd(
    ...
    from flora.plotting.create_plots import create_plots
...
def check_cmd(
    ...
    from flora.plotting.check_plots import check_plots
...
def add_cmd(ctx: click.Context, final_dir: str):
    from flora.plotting.plot_tools import add_plot_directory
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from flora.plotting.plot_tools import remove_plot_directory

...
from flora.cmds.units import units
...
from flora.util.misc import format_bytes
from flora.util.misc import format_minutes
from flora.util.network import is_localhost
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total flora farmed: {amounts['farmed_amount'] / units['flora']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['flora']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['flora']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'flora start wallet' and 'chia wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'flora wallet show'")
    else:
        print("Note: log into your key using 'flora wallet show' to see rewards for each key")

...
from flora.consensus.coinbase import create_puzzlehash_for_pk
from flora.ssl.create_ssl import generate_ca_signed_cert, get_flora_ca_crt_key, make_ca_cert
from flora.util.bech32m import encode_puzzle_hash
from flora.util.config import (
    create_default_flora_config,
    initial_config_file,
    load_config,
    save_config,
...
from flora.util.path import mkdir
from flora.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'flora keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "xfl_target_address" not in config["farmer"]
    stop_searching_for_pool = "xfl_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xfl_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xfl_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "xfl_target_address" not in config["farmer"]:
        print(f"Setting the xfl destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"]["xfl_target_address"] = all_targets[0]
    elif config["farmer"]["xfl_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['xfl_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xfl_target_address" not in config["pool"]:
        print(f"Setting the xfl destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["xfl_target_address"] = all_targets[0]
    elif config["pool"]["xfl_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xfl_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root: Path):
    ...
    flora_ca_crt, flora_ca_key = get_flora_ca_crt_key()
    flora_ca_crt_path = ca_dir / "flora_ca.crt"
    flora_ca_key_path = ca_dir / "flora_ca.key"
    flora_ca_crt_path.write_bytes(flora_ca_crt)
    flora_ca_key_path.write_bytes(flora_ca_key)
    ...
    flora_ca_crt, flora_ca_key = get_flora_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, flora_ca_crt, flora_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if flora_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return flora_init(root_path)


def flora_version_number() -> Tuple[str, str, str, str]:
...
def flora_minor_release_number():
    res = int(flora_version_number()[2])
    print(f"Install release number: {res}")
    return res


def flora_full_version_str() -> str:
    major, minor, patch, dev = flora_version_number()
...
def flora_init(root_path: Path):
    ...
    create_default_flora_config(root_path)
    create_all_ssl(root_path)
    check_keys(root_path)
    print("")
    print("To see your keys, run 'flora keys show --show-mnemonic-seed'")

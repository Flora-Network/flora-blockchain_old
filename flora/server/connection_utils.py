...
async def send_all_first_reply(
    ...
    async def do_func(peer_x: WSFloraConnection, func_x: str, arg_x: Any):
...
async def send_to_random(func: str, arg: Any, peers: List[WSFloraConnection]) -> Optional[Tuple[Any, WSFloraConnection]]:
    """performs an API request to peers and returns the result of the first response and the peer that sent it."""

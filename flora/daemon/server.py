...
from flora.cmds.init_funcs import flora_init
from flora.daemon.windows_signal import kill
from flora.server.server import ssl_context_for_root, ssl_context_for_server
from flora.ssl.create_ssl import get_mozilla_ca_crt
...
from flora.util.service_groups import validate_service
from flora.util.setproctitle import setproctitle
from flora.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Flora.")
    quit()
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("flora daemon exiting in 5 seconds")
...
async def async_run_daemon(root_path: Path) -> int:
    flora_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("flora_daemon")

...
from flora.pools.pool_config import PoolWalletConfig, load_pool_config
from flora.protocols import farmer_protocol, harvester_protocol
...
from flora.util.config import load_config, save_config, config_path_for_filename
...
from flora.wallet.derive_keys import (
...
from flora.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    def __init__(
        ...
        if len(self.get_public_keys()) == 0:
            error_str = "No keys exist. Please run 'flora keys generate' or open the UI."
        ...
        self.pool_target_encoded = pool_config["xfl_target_address"]
    ...
    async def get_cached_harvesters(self, connection: WSFloraConnection) -> Optional[Tuple[Dict, float]]:

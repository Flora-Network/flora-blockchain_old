...
def get_block_challenge(
    ...
    if len(header_block.finished_sub_slots) > 0:
        ...
        if genesis_block:
            ...
            if overflow:
                if skip_overflow_last_ss_validation:
                    # Overflora infusion without the new slot, so get the last challenge
                    challenges_to_look_for = 1
                else:
                    # Overflora infusion, so get the second to last challenge. skip_overflow_last_ss_validation is False,

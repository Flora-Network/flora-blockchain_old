...
from flora.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from flora.protocols import wallet_protocol
from flora.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from flora.protocols.protocol_message_types import ProtocolMessageTypes
from flora.protocols.wallet_protocol import (
...
from flora.server.node_discovery import WalletPeers
...
from flora.util.errors import Err, ValidationError
from flora.util.ints import uint32, uint128
from flora.util.keychain import Keychain
from flora.util.lru_cache import LRUCache
from flora.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from flora.wallet.settings.settings_objects import BackupInitialized
...
from flora.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'flora keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSFloraConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSFloraConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSFloraConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSFloraConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSFloraConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSFloraConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
